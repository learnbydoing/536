...
//case class StartMultipleActors(token:String, fPath:String, sPath:String, tPath:String)
//case class SendMultipleActors(token:String, fPath:String, sPath:String, tPath:String)
...
class First extends Actor {
   ...
   def receive = {
        ...
        if(token == "T") 
	{
	   ...
	   println("tCount for Actor 1 is: " + tCountFirst)
        }
        else
        {
	   next = context.actorOf(Props[Third], name="third")
	   uCountFirst = First.incU
	   println("uCount for Actor 1 is: " + uCountFirst)
        }	
        next ! Send(token)         
      case Start(token) =>
	Thread.sleep(500)
...
class Second extends Actor {
   ...
   def receive = {
	...
	Thread.sleep(500)
	   ...
	   println("tCount for Actor 2 is: " + tCountSecond)
        }
        else
        {
           next = context.actorOf(Props[First], name="first")
           uCountSecond = Second.incU
           println("uCount for Actor 2 is: " + uCountSecond)
        }
	next ! Send(token)
}// End receive()
}//End class Second
...
class Third extends Actor {
   ...
   def receive = {
        ...
        if(token == "T")
	{
	  ...
	  println("tCount for Actor 3 is: " + tCountThird)
        }
        else
        {
           next = context.actorOf(Props[Second], name="second")
           uCountThird = Third.incU
           println("uCount for Actor 3 is: " + uCountThird)
        }
       next ! Send(token)
}// End receive()
}//End class Third
...
//	first ! StartMultipleActors("T", first.path.toString, second.path.toString, third.path.toString)
//	first ! StartMultipleActors("U", first.path.toString, second.path.toString, third.path.toString)
